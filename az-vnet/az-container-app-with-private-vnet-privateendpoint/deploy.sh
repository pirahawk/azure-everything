...
# Notice how we are waiting for VNET deploy to finish. Because the two deployments below will need it to be in place.
    az deployment group create -g $azGroupName -n "vnet-$azDeploymentName" -f ./az-vnet/az-container-app-with-private-vnet-privateendpoint/bicep/deployvnet.bicep \
    --parameters \
...
az deployment group create -g $azGroupName -n "vm-$azDeploymentName" -f ./az-vnet/az-container-app-with-private-vnet-privateendpoint/bicep/deployVm.bicep \
 --parameters \
 randomSuffix="$randomSuffix" \
 userPrincipalId="$azSignedInUserId" \
...
az deployment group create -g $azGroupName -n "services-$azDeploymentName"  -f ./az-vnet/az-container-app-with-private-vnet-privateendpoint/bicep/deployPrivateEndpointResources.bicep \
 --parameters \
 randomSuffix="$randomSuffix" \
 userPrincipalId="$azSignedInUserId"

# az deployment group create -g $azGroupName -n "app-$azDeploymentName"  -f ./az-vnet/az-container-app-with-private-vnet-privateendpoint/bicep/deployContainerApps.bicep \
#  --parameters \
#  randomSuffix="$randomSuffix" \
#  userPrincipalId="$azSignedInUserId"
...
then
 ...
 # need to wait for this as the private DNS Zone depends on the resources created here
# az deployment group create -g $azGroupName -n "dnszone-$azDeploymentName"  -f ./az-vnet/az-container-app-with-private-vnet-privateendpoint/bicep/deployDns.bicep \
#  --parameters \
#  randomSuffix="$randomSuffix" \
#  userPrincipalId="$azSignedInUserId" \
